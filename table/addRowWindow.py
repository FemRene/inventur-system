...
def openWindow(table: Treeview):
    ...
    entry_id.insert(0, "Is set Automatically")
    ...
    # Add save Button
    save = tk.Button(root, text="Save", command=lambda: addToTree(root, table, highest_id+1, entry_service_tag.get(), entry_type.get(), entry_raum.get()))
...
def addToTree(window: tk.Tk, table: Treeview, id, serviceTag, typ, raum):
    ...
    databaseManager.insertNewRow(serviceTag,typ,raum)
    window.destroy()